...
var actions = {
    ...
    "@": function (node) {
         ...
         */
        if (helper) {
            return helper(node, utils.compileParams(node[1].params, context), context, processAst);
        }
    ...
    "#": function (node) {
        ...
        var done = false;
        var out  = "";
        var params = utils.compileParams(node.params, context);
        var append = "";

        if (params.sep) {
            append = separators[params.sep]();
        }
        ...
        if (item) {
            ...
            if (Array.isArray(item)) {
                context._of = item.length;
                _.each(item, function (_item, i) {
                    context.advanceHead(i);
                    out += processAst(node.body, false);
                    if (!context.isLast()) {
                        out += append;
                    }
...
function processString (string) {
    return processAst(parser.parse(string), true);
}
